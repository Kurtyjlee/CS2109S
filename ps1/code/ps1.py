...
def mnc_tree_search(m: int, c: int):
    ...
    class State:
        def __init__(
            self, m: int, c: int, direction: int, steps: tuple = tuple()
        ) -> None:
            self.steps = steps
            self.left_m = m
            self.left_c = c
    ...
    def action(state: State) -> list:
        ...
        for possible in possiblities:
            ...
            if not (
                new_left_m < 0 or new_left_c < 0 or new_left_m > m or new_left_c > c
            ):
...
def mnc_graph_search(m, c):
    ...
    visited = [
        [[False for _ in range(m + 1)] for _ in range(c + 1)] for _ in range(2)
    ]  # m cols, c rows, 2 depth
    initial_state = State(m, c, -1)
    frontier = queue.Queue()
    frontier.put(initial_state)
    new_upper_limit = (m + 1) * (c + 1) * 2
...
def pitcher_search(p1, p2, p3, a):
    ...
    while queue:
        ...
        for i in range(3):
            for j in range(3):
                amount_to_pour = current_state[j].find_amount_to_pour(
                    current_state[i].volume
                )
